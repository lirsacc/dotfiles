...
# Based off: https://github.com/mathiasbynens/dotfiles/blob/master/.osx
...
# Set sidebar icon size to medium (use 3 for large)
...
sudo rm /private/var/vm/sleepimage &> /dev/null || true
sudo touch /private/var/vm/sleepimage &> /dev/null || true
...
# Four-letter codes for the other view modes: `icnv`, `Nlsv`, `glyv`, `Flwv`
...
if [[ -n $(xattr -l ~/Library | grep com.apple.FinderInfo) ]]; then
...
# Use Pro theme by default in Terminal.app
...
sudo defaults write com.apple.mail DraftsViewerAttributes -dict-add "SortedDescending" -string "no"
...
# Disable Time Machine backups alltogether
